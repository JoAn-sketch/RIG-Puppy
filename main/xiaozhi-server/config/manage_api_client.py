...
async def report_running_firmware_version(
    device_id: str, version: str
) -> Optional[Dict]:
    """Report the version that a connected device is actually running.

    This deliberately bypasses the retrying client path: version reporting must
    never delay MCP initialization or realtime conversation startup.
    """
    normalized_device_id = str(device_id or "").strip()
    normalized_version = str(version or "").strip()
    if not normalized_device_id or not normalized_version or not ManageApiClient._instance:
        return None
    try:
        return await ManageApiClient._instance._async_request(
            "POST",
        ...
        print(
            "上报设备实际固件版本失败: "
            f"device_id={normalized_device_id}, version={normalized_version}, error={e}"
        )
        return None
