...
def get_news_from_newsnow(
    ...
    try:
        ...
        if url and url != "#":
            try:
                detail_content = fetch_news_detail(url)
                if detail_content == "无法获取详细内容":
                    detail_content = ""
            except Exception as e:
                logger.bind(tag=TAG).warning(f"抓取详情失败，仅播报标题: {e}")
                detail_content = ""

        if detail_content:
            # 控制喂给 LLM 的长度，避免超 token
            snippet = detail_content[:1500]
            news_report = (
                f"根据下列数据，用{lang}回应用户的新闻查询请求：\n\n"
                f"新闻标题: {title}\n"
                f"正文摘要: {snippet}\n\n"
                f"(请把这条新闻完整讲给用户听：先说标题，再用自然口语 3-5 句话讲清来龙去脉。"
                f"不要只念标题、不要问用户要不要详情、不要提及'总结''摘要'等字眼。)"
            )
        else:
            news_report = (
                f"根据下列数据，用{lang}回应用户的新闻查询请求：\n\n"
                f"新闻标题: {title}\n"
                f"(只有标题、抓不到正文。请直接把标题告诉用户，自然口语化即可，不要追问。)"
            )
