...
from config.manage_api_client import report_running_firmware_version
    ...
    if "result" in payload:
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            if isinstance(server_info, dict):
                ...
                device_id = str(
                    getattr(conn, "headers", {}).get("device-id", "")
                ).strip()
                if device_id and version:
                    # The MCP initialize response is emitted after every device
                    # WebSocket reconnect, including a reboot after OTA.
                    asyncio.create_task(
                        report_running_firmware_version(device_id, version)
                    )
