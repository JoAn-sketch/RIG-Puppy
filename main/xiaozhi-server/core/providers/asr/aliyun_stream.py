                ...
                try:
                    ...
                    if message_name == "TranscriptionStarted":
                        ...
                        if text:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # Streaming ASR can emit a SentenceEnd for a short
                                # sentence fragment before local VAD sees that the
                                # user really stopped. Keep the latest final text,
                                # but only close the turn once voice_stop is true.
                                self._set_pending_final(conn, text, audio_data)
                                if conn.client_voice_stop:
                                    await self._finalize_pending(
                                        conn, audio_data, "aliyun_sentence_end_voice_stop"
                                    )
                                    break
                                logger.bind(tag=TAG).info(
                                    "[ASR-Turn] defer aliyun SentenceEnd until local VAD voice_stop"
                                )
                    ...
                    if (
                        ...
                        reason = (
                            ...
                            if conn.client_voice_stop
                            else "aliyun_sentence_end_grace_timeout"
                        )
