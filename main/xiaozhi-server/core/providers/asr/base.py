...
class ASRProviderBase(ABC):
    ...
    def _ensure_turn_audio_stats(self, conn: "ConnectionHandler"):
        if not hasattr(conn, "asr_turn_audio_stats"):
            conn.asr_turn_audio_stats = {}
        stats = conn.asr_turn_audio_stats
        if not stats:
            stats.update(
                {
                    ...
                    "upstream_packets": 0,
                    "upstream_bytes": 0,
                    "upstream_cached_packets": 0,
                    "upstream_cached_bytes": 0,
                    "upstream_send_errors": 0,
                    "final_reason": "",
                    "final_text": "",
                }
            )
    ...
    def track_received_audio(
        self,
        conn: "ConnectionHandler",
        audio: bytes,
        audio_have_voice: bool,
    ):
        stats = self._ensure_turn_audio_stats(conn)
        stats["received_packets"] += 1
        stats["received_bytes"] += len(audio or b"")
        if audio_have_voice:
            now = time.time()
            stats["voice_packets"] += 1
            if not stats.get("first_voice_at"):
                stats["first_voice_at"] = now
            stats["last_voice_at"] = now

    def track_upstream_audio(
        self,
        conn: "ConnectionHandler",
        pcm_frame: bytes,
        cached: bool = False,
        ...
        stats = self._ensure_turn_audio_stats(conn)
        pcm_len = len(pcm_frame or b"")
        if cached:
            stats["upstream_cached_packets"] += 1
            stats["upstream_cached_bytes"] += pcm_len
        else:
            stats["upstream_packets"] += 1
            stats["upstream_bytes"] += pcm_len

    def track_upstream_error(self, conn: "ConnectionHandler"):
        stats = self._ensure_turn_audio_stats(conn)
        stats["upstream_send_errors"] += 1

    def log_asr_turn_summary(
        self,
        conn: "ConnectionHandler",
        ...
        stats = self._ensure_turn_audio_stats(conn)
        text_preview = text
        if isinstance(text_preview, dict):
            text_preview = text_preview.get("content", "")
        text_preview = str(text_preview or "").strip()
        stats["final_reason"] = reason
        stats["final_text"] = text_preview
        started_at = float(stats.get("started_at") or time.time())
        first_voice_at = float(stats.get("first_voice_at") or 0)
        last_voice_at = float(stats.get("last_voice_at") or 0)
        voice_duration_ms = 0
        if first_voice_at and last_voice_at and last_voice_at >= first_voice_at:
            voice_duration_ms = int((last_voice_at - first_voice_at) * 1000)
        duration_ms = int((time.time() - started_at) * 1000)
        logger.bind(tag=TAG).info(
            "[ASR-Turn] "
            f"turn={stats.get('turn_id')} reason={reason} "
            f"received_packets={stats.get('received_packets', 0)} "
            f"received_bytes={stats.get('received_bytes', 0)} "
            f"voice_packets={stats.get('voice_packets', 0)} "
            f"voice_duration_ms={voice_duration_ms} "
            f"buffer_packets={len(asr_audio_task or [])} "
            f"upstream_packets={stats.get('upstream_packets', 0)} "
            f"upstream_bytes={stats.get('upstream_bytes', 0)} "
            f"cached_packets={stats.get('upstream_cached_packets', 0)} "
            f"send_errors={stats.get('upstream_send_errors', 0)} "
            f"turn_duration_ms={duration_ms} "
            f"text={text_preview}"
        )
    ...
    async def receive_audio(self, conn: "ConnectionHandler", audio, audio_have_voice):
        self.track_received_audio(conn, audio, audio_have_voice)
    ...
    async def handle_voice_stop(self, conn: "ConnectionHandler", asr_audio_task: List[bytes]):
        ...
        try:
            ...
            stats = self._ensure_turn_audio_stats(conn)
            if hasattr(conn, "begin_latency_trace"):
                conn.begin_latency_trace(turn_id=stats.get("turn_id"))
                conn.mark_latency_stage(
                    "asr_processing_start",
                    buffer_packets=len(asr_audio_task or []),
                )
            ...
            self.log_asr_turn_summary(
                conn,
                asr_audio_task,
                raw_text,
                getattr(conn, "asr_final_reason", "") or "voice_stop",
            )
            if hasattr(conn, "update_latency_trace"):
                conn.update_latency_trace(user_text=raw_text)
                conn.mark_latency_stage(
                    "asr_final",
                    text=raw_text,
                    asr_processing_ms=int((time.monotonic() - total_start_time) * 1000),
                )
