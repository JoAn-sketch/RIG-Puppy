...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        final_grace_ms = config.get("final_sentence_grace_ms", 500)
        self.final_sentence_grace_ms = int(final_grace_ms) if final_grace_ms else 500
    ...
    def _set_pending_final(self, conn: "ConnectionHandler", text: str, audio_data):
        self._pending_final_text = self._merge_final_text(text)
    ...
    def _pending_final_ready(self, conn: "ConnectionHandler") -> bool:
        if not self._pending_final_text or conn.client_listen_mode == "manual":
            return False
        if conn.client_voice_stop:
            return True
        if self.final_sentence_grace_ms <= 0 or not self._pending_final_at:
            return False
        elapsed_ms = (asyncio.get_running_loop().time() - self._pending_final_at) * 1000
        if elapsed_ms < self.final_sentence_grace_ms:
            return False
        stats = getattr(conn, "asr_turn_audio_stats", {}) or {}
        voice_packets = int(stats.get("voice_packets") or 0)
        if voice_packets > self._pending_final_voice_packets:
            # User continued speaking after the ASR final event. Restart the
            # grace window and wait for the next final/silence boundary.
            self._pending_final_at = asyncio.get_running_loop().time()
            ...
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    # 处理result-generated事件
                        ...
                        if is_final:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # DashScope may mark a short fragment as a final
                                # sentence while the user is still speaking. Treat
                                # it as the latest candidate and wait for local VAD.
                                self._set_pending_final(conn, text, audio_data)
                                if conn.client_voice_stop:
                                    await self._finalize_pending(
                                        conn, audio_data, "aliyunbl_sentence_end_voice_stop"
                                    )
                                    break
                                logger.bind(tag=TAG).info(
                                    "[ASR-Turn] defer aliyunbl final sentence until local VAD voice_stop"
                                )
                    ...
                    if (
                        ...
                        and self._pending_final_text
                        and self._pending_final_ready(conn)
                    ):
                        reason = (
                            "local_voice_stop_after_sentence_end"
                            if conn.client_voice_stop
                            else "aliyunbl_sentence_end_grace_timeout"
                        )
                    ...
                    if (
                        ...
                        reason = (
                            "local_voice_stop_recv_timeout"
                            if conn.client_voice_stop
                            else "aliyunbl_sentence_end_grace_timeout"
                        )
                        await self._finalize_pending(conn, conn.asr_audio, reason)
                        break
                    continue
