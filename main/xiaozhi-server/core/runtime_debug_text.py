...
from core.conversation_result import ConversationResult
...
class DebugRuntimeTransport:
    def __init__(self):
        ...
        self._latest_conversation_result: Dict[str, Any] = {}
    ...
    async def begin_turn(self):
        async with self._turn_lock:
            ...
            self._latest_conversation_result = {}
            self._latest_llm_text = ""
            self._latest_reply_text = ""
            self._turn_completed = False
    ...
    async def send(self, payload):
        ...
        if message_type == "runtime_debug":
            self._latest_runtime_debug = {
                ...
                "conversation_lifecycle": message.get("conversation_lifecycle"),
                "conversation_request": message.get("conversation_request"),
                "conversation_result": message.get("conversation_result"),
            ...
            self._latest_conversation_result = (
                message.get("conversation_result") or {}
            )
            if message.get("stage") in {"post_reply", "result_ready"}:
    ...
    def _maybe_finish_result(self):
        ...
        if not self._turn_completed:
            return
        if not self._latest_conversation_result:
            return
        ...
        self._result_future.set_result(
            {
                "reply": reply_text,
                "runtime_debug": dict(self._latest_runtime_debug or {}),
                "conversation_result": dict(self._latest_conversation_result or {}),
            }
        )
    ...
    def capture_reply(self, reply_text, runtime_debug, loop):
        ...
        self._latest_conversation_result = (
            (self._latest_runtime_debug or {}).get("conversation_result") or {}
        )
        self._turn_completed = True
        if loop is not None:
...
class RuntimeDebugTextSession:
    ...
    def _build_result_from_conn(self, conn: ConnectionHandler, dialogue_len_before: int):
        conversation_result = getattr(conn.session_state, "last_conversation_result", None)
        if isinstance(conversation_result, ConversationResult):
            result_dict = conversation_result.to_dict()
            return {
                "reply": conversation_result.response,
                "runtime_debug": conn._build_runtime_debug_payload(),
                "conversation_result": result_dict,
            }
        ...
        return {
            ...
            "conversation_result": (runtime_debug or {}).get("conversation_result") or {},
        }
