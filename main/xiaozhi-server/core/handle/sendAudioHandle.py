...
async def _do_send_audio(conn: "ConnectionHandler", opus_packet, flow_control):
    ...
    if packet_index == 0 and hasattr(conn, "mark_latency_stage"):
        conn.mark_latency_stage(
            "audio_play_start",
            packet_index=packet_index,
            opus_bytes=len(opus_packet or b""),
        )
    ...
    if hasattr(conn, "mark_latency_stage"):
        conn.mark_latency_stage(
            "device_first_audio_packet_sent",
            packet_index=packet_index,
            opus_bytes=len(opus_packet or b""),
        )
...
async def send_tts_message(conn: "ConnectionHandler", state, text=None):
    ...
    if state == "stop":
        ...
        if hasattr(conn, "mark_latency_stage"):
            conn.mark_latency_stage("audio_play_end")
        if hasattr(conn, "mark_latency_stage"):
            conn.mark_latency_stage("tts_stop_sent")
