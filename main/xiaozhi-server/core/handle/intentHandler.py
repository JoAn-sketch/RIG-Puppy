...
from core.conversation_result import ResponseType
        ...
        if "function_call" in intent_data:
            ...
            if function_name == "result_for_context":
                ...
                def process_context_result():
                    ...
                    conn._conversation_result_type_hint = ResponseType.DIRECT_ANSWER
            ...
            def process_function_call():
                ...
                if result:
                    ...
                    if result.action == Action.RESPONSE:  # 直接回复前端
                        ...
                        if text is not None:
                            conn._conversation_result_type_hint = ResponseType.TOOL_RESULT
                            speak_txt(conn, text)
...
def speak_txt(conn: "ConnectionHandler", text):
    ...
    if hasattr(conn, "_build_conversation_result"):
        response_type = getattr(
            conn,
            "_conversation_result_type_hint",
            ResponseType.DIRECT_ANSWER,
        )
        conn._build_conversation_result(text, response_type=response_type)
        conn._conversation_result_type_hint = ResponseType.DIRECT_ANSWER
