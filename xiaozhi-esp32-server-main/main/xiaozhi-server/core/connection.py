...
class ConnectionHandler:
    ...
    def _init_prompt_enhancement(self):
        ...
        try:
            agent_id = self.config.get("agent_id") or self.config.get("id")
            if agent_id:
                ...
                try:
                    with db.cursor() as cursor:
                        # 先查时间段调度，看当前时间是否匹配某个模式
                        from datetime import datetime
                        now_time = datetime.now().strftime("%H:%M:%S")
                        cursor.execute(
                            "SELECT m.system_prompt, m.context_modules, m.knowledge_ids, m.mode_name "
                            "FROM rl_mode_schedule s "
                            "JOIN ai_agent_mode m ON s.mode_id = m.id "
                            "WHERE s.agent_id = %s AND s.is_active = 1 "
                            "AND %s BETWEEN s.start_time AND s.end_time "
                            "LIMIT 1",
                            (agent_id, now_time)
                        )
                        scheduled_mode = cursor.fetchone()

                        if scheduled_mode:
                            default_mode = scheduled_mode
                            self.logger.bind(tag=TAG).info(
                                f"时间段调度匹配: {scheduled_mode.get('mode_name')}"
                            )
                        else:
                            # fallback: 使用默认模式
                            cursor.execute(
                                "SELECT system_prompt, context_modules, knowledge_ids, mode_name FROM ai_agent_mode "
                                "WHERE agent_id = %s AND is_default = 1 LIMIT 1",
                                (agent_id,)
                            )
                            default_mode = cursor.fetchone()
                        if default_mode:
                            mode_prompt = default_mode.get("system_prompt")
                            raw = default_mode.get("context_modules")
                            if raw:
                                if isinstance(raw, str):
                                    ...
                                    context_modules = raw
                            # 设置默认模式的知识库列表
                            kid_raw = default_mode.get("knowledge_ids")
                            if kid_raw:
                                if isinstance(kid_raw, str):
                                    self._current_knowledge_ids = [str(k) for k in json.loads(kid_raw)]
                                elif isinstance(kid_raw, list):
                                    self._current_knowledge_ids = [str(k) for k in kid_raw]
                            self.logger.bind(tag=TAG).info(
                                f"加载默认模式, modules: {context_modules}, "
                                f"knowledge_ids: {getattr(self, '_current_knowledge_ids', [])}"
                            )
                finally:
                    db.close()
        except Exception as e:
            self.logger.bind(tag=TAG).warning(f"查询默认模式失败，使用无模式渲染: {e}")
